...
namespace Umbra.Implementations
{
	...
	static public class RenderHelp
	{
		...
		static public void CreateBlockData3DArray(out int DataID)
		{
			DataID = GL.GenTexture();
			GL.BindTexture(TextureTarget.Texture3D, DataID);

			GL.TexImage3D(TextureTarget.Texture3D, 0, PixelInternalFormat.Luminance,
				Constants.World.ChunkSize * Constants.World.WorldSize, Constants.World.ChunkSize * Constants.World.WorldSize, Constants.World.ChunkSize * Constants.World.WorldSize,
				0, OpenTK.Graphics.OpenGL.PixelFormat.Luminance, PixelType.UnsignedByte, IntPtr.Zero);
			GL.TexEnv(TextureEnvTarget.TextureEnv, TextureEnvParameter.TextureEnvMode, Color.Transparent);
